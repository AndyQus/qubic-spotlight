...
appear on a public dashboard (incl. Qubic network metrics) and can be embedded as
an overlay banner into any website via a small code snippet.
...
| `Marketing` | manage/delete all ads |
| `Ecosystem` | only own ads (max. 5 active) |
...
Management: `/api/admin/ads` (Admin+Marketing), `/api/admin/users` (Admin).
