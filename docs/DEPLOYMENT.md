...
services:
  qubic_spotlight:
    ...
    environment:
      ...
      # Optional: Länder-Statistik (Admin → Besucher). Pfad zur lokalen db-ip
      # "IP to Country Lite"-DB (MMDB, kein Account); fehlt sie, zählen Besuche
      # ohne Land. Die IP wird nie gespeichert, nur der anonymisierte Ländercode.
      # Setup + Download-Skript s. VPS-SETUP-SICHER.md (10d).
